...
def createTempTable(conn, icmdlist):
	...
	with conn.cursor() as cursor:
		cursor.execute(f"""
			...
			ALTER TABLE tempTable ADD PRIMARY KEY (CensusTract);
			CREATE INDEX idx_tempTable_State ON tempTable (State);
		""")
...
def main():
	...
	if CreateDB:
		createTable(conn)
		#createTempTable(conn, cmdlist)
	
	loadBatch(conn, cmdlist)
	#load(conn, cmdlist)
